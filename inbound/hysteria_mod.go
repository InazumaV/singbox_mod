...
var _ adapter.Inbound = (*HysteriaM)(nil)

type HysteriaM struct {
	myInboundAdapter
	quicConfig   *quic.Config
	tlsConfig    tls.ServerConfig
	users        map[string]string // k:auth v:name
	xplusKey     []byte
	sendBPS      uint64
	recvBPS      uint64
...
func (h *HysteriaM) accept(ctx context.Context, conn quic.Connection) error {
	...
	if len(h.users) > 0 {
		n, ok := h.users[string(clientHello.Auth)]
		if !ok {
...
func (h *HysteriaM) udpRecvLoop(conn quic.Connection) {
	for {
...
func (h *HysteriaM) acceptStream(ctx context.Context, conn quic.Connection, stream quic.Stream) error {
...
func (h *HysteriaM) AddUsers(users []option.HysteriaUser) error {
	for i := range users {
		var a string
		if len(users[i].Auth) > 0 {
			a = string(users[i].Auth)
		} else {
			a = users[i].AuthString
		}
		h.users[a] = users[i].Name
	}
	return nil
}

func (h *HysteriaM) DelUsers(auths []string) error {
	for _, u := range auths {
		delete(h.users, u)
	}
	return nil
